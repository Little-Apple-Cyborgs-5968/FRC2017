...
public class PositionTracker {
	...
	private static final double STRAIGHT_LINE_THRESHOLD = .025;
	...
	private static void updateCoordinates(){
		double xDisplacementFromAccel = NavXMXP.getDisplacementX();
		double yDisplacementFromAccel = NavXMXP.getDisplacementY();
		double leftEncoderDistance = DriveBase.getLeftDistance();
		double rightEncoderDistance = DriveBase.getRightDistance();
		double angleUnprocessed = NavXMXP.getYaw() * Math.PI / 180; //angle from -pi / 2 to pi / 2
		double angle = angleUnprocessed;
		double avgDistance;
		...
		if((Math.abs(leftEncoderDistance) >= MOVEMENT_THRESHOLD || Math.abs(rightEncoderDistance) >= MOVEMENT_THRESHOLD) && 
			...
			if(Math.abs(leftEncoderDistance) - Math.abs(rightEncoderDistance) < STRAIGHT_LINE_THRESHOLD){
			...
			//approximate path as a circular arc
				...
				if(Math.abs(leftEncoderDistance) - Math.abs(rightEncoderDistance) > MOVEMENT_THRESHOLD){
					return;
				}
		...
		else if((Math.abs(leftEncoderDistance) < MOVEMENT_THRESHOLD && Math.abs(rightEncoderDistance) < MOVEMENT_THRESHOLD) &&
				(Math.abs(xDisplacementFromAccel) >= MOVEMENT_THRESHOLD || Math.abs(yDisplacementFromAccel) >= MOVEMENT_THRESHOLD)){
		...
		previousAngle = angleUnprocessed;
